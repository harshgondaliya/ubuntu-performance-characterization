...
void main(){    
    outputFile = fopen("remote-seq-file-read.txt", "w");
    ...
    while(id <= NO_OF_FILES){
        ...
        char buf[4096];
        sprintf(file_name, "/home/harsh/www/test-%d.img",(id));
        int fd = open(file_name, __O_DIRECT|O_SYNC);
...
//            int e = system("sync; echo 3 > /proc/sys/vm/drop_caches");
 //           posix_fadvise(fd, 0, file_sizes[id-1], POSIX_FADV_DONTNEED);
            ...
            if(end_time - start_time < 0){
                ...
                total_time = total_time + (unsigned long long int)(end_time - start_time); 
            }
        }
//        printf("total_time per block: %lf\n",(total_time*0.344/(NO_OF_ITERATIONS*chunks)));
        fprintf(outputFile, "%ld, %lf\n", (chunks), (total_time*0.344/(NO_OF_ITERATIONS*chunks)));
        c = buf[0];
        id+=1;
    }
    fclose(outputFile);
