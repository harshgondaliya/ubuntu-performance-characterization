...
def main():
    ...
    PIPE_OVERHEAD = float(sys.argv[6])
    PTHREAD_OVERHEAD = float(sys.argv[7])

    f = open(fname, "r")
    data = []
    lines = f.readlines()
    for line in lines:
        x = line.rstrip('\n') # only one time pthread call is made
        x_minus_overhead = ((float(x) - READ_OVERHEAD - (LOOP_OVERHEAD*NO_OF_ITERATIONS)- (PIPE_OVERHEAD * NO_OF_ITERATIONS) - PTHREAD_OVERHEAD))/(2*NO_OF_ITERATIONS)
