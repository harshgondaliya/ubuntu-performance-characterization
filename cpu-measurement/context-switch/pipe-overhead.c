...
void main(){    
    ...
    for(int i=0;i<NO_OF_TRIALS;i++){
        ...
        for(int j=0; j<NO_OF_ITERATIONS; j++){
            ...
            read(p_to_c[0], readBuf, (strlen(token)+1));
            write(c_to_p[1], readBuf, (strlen(token)+1));
            read(c_to_p[0], readBuf2, (strlen(token)+1));
        }
        asm volatile("RDTSCP\n\t"
                    "mov %%edx, %0\n\t"
                    "mov %%eax, %1\n\t"
    ...
    outputFile = fopen("pipe-overhead.txt", "w");
    for(int i=0; i<NO_OF_TRIALS; i++){
