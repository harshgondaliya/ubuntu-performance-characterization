...
def main():
    ...
    for line in lines:
        x = line.rstrip('\n')
        x_minus_overhead = ((float(x) - READ_OVERHEAD - (LOOP_OVERHEAD*NO_OF_ITERATIONS)- (PIPE_OVERHEAD * NO_OF_ITERATIONS) ))/(2*NO_OF_ITERATIONS) # in one iteration two context switch are there
